      ...
      <header className="bg-white border-b border-gray-200 px-4 h-14 flex items-center justify-between shrink-0 z-20">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/settings/notifications"
            className="w-9 h-9 rounded-xl bg-gray-50 flex items-center justify-center text-gray-600 hover:text-primary hover:bg-gray-100 transition-colors">
            <BellIcon />
          </Link>
        ...
        <aside className="hidden md:flex flex-col w-[72px] bg-white border-r border-gray-200 py-3 shrink-0">
          {sidebarNav.map(item => (
            <Link key={item.href} href={item.href}
              className={`flex flex-col items-center gap-1.5 py-3 mx-1.5 rounded-xl transition-colors ${
                item.active ? 'bg-primary/5 text-primary' : 'text-gray-600 hover:bg-background hover:text-primary'
              }`}>
              {item.icon}
          ...
          {isAdmin && (
            <Link href="/admin/club-settings"
              className={`flex flex-col items-center gap-1.5 py-3 mx-1.5 rounded-xl transition-colors ${
                starts('/admin') ? 'bg-primary/5 text-primary' : 'text-gray-600 hover:bg-background hover:text-primary'
              }`}>
              <SettingsIcon />
        ...
        {tabs.map(tab => (
          <Link key={tab.href} href={tab.href}
            className={`flex-1 flex flex-col items-center justify-center gap-1 py-2.5 transition-colors ${
              tab.active ? 'text-primary' : 'text-gray-600 hover:text-primary'
            }`}>
            {tab.icon}
