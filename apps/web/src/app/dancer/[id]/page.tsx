        ...
        <aside className="hidden md:flex flex-col w-[72px] bg-white border-r border-gray-200 py-3 shrink-0">
          {sidebarNav.map(item => (
            <Link key={item.href} href={item.href}
              className="flex flex-col items-center gap-1.5 py-3 mx-1.5 rounded-xl text-gray-600 hover:bg-background hover:text-primary transition-colors">
              {item.icon}
          ...
          <button onClick={async () => { await logout(); router.replace('/login'); }}
            className="flex flex-col items-center gap-1.5 py-3 mx-1.5 rounded-xl text-gray-600 hover:bg-red-50 hover:text-red-500 transition-colors">
            <LogoutIcon />
                ...
                <div className="flex flex-col sm:grid sm:grid-cols-2 gap-3">
                  {[...quickCards, { href: '/membership', label: 'Cotisation', icon: <CardIcon className="w-6 h-6" />, bg: 'bg-orange' }].map(card => (
                    <Link key={card.href} href={card.href}
                      ...
                      <CardWaves />
                      <p className="relative text-base font-bold leading-snug">{card.label}</p>
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 flex z-10">
        {tabs.map(tab => (
          <Link key={tab.href} href={tab.href}
            className="flex-1 flex flex-col items-center justify-center gap-1 py-2.5 text-gray-600 hover:text-primary transition-colors">
            {tab.icon}
