...
export default function NewBankDepositPage() {
  ...
  useEffect(() => {
    getDocs(query(collection(db, 'bankAccounts'), orderBy('name'))).then(snap => {
      const accounts: BankAccount[] = snap.docs.map(d => ({
        id: d.id, name: d.data().name ?? '', bank: d.data().bank ?? '',
        accountNumber: d.data().accountNumber ?? '', holder: d.data().holder ?? '', label: d.data().label ?? '',
      }));
  ...
  // Load installments when method changes
  const loadRows = useCallback(async (method: PaymentMethod) => {
    setLoading(true);
    setLoadError(null);
    setRows([]);
    setSelected(new Set());
    try {
      const installSnap = await getDocs(query(
      ...
      const loadedRows: InstallmentRow[] = await Promise.all(eligible.map(async (d) => {
        const data = d.data();
        let memberName: string | undefined;

        // For group installments, show all dancers
        ...
        return {
          ...
          draweeBank: data.draweeBank ?? undefined,
          draweeCity: data.draweeCity ?? undefined,
          transferRef: data.transferRef ?? undefined,
          transferDate: data.transferDate ?? undefined,
          cashReceiptRef: data.cashReceiptRef ?? undefined,
          cashDate: data.cashDate ?? undefined,
        };
      }));
  ...
  useEffect(() => { loadRows(paymentMethod); }, [paymentMethod, loadRows]);
  ...
  const generateAndDeposit = async () => {
    ...
    try {
      ...
      // Titre
      page.drawText(PDF_TITLE[paymentMethod], {
      ...
      if (selectedBankAccount.label) { y -= 12; page.drawText(selectedBankAccount.label, { x: 50, y, size: 9, font, color: rgb(0.5, 0.5, 0.5) }); }
      y -= 20;

      // Date
      const dateFr = new Date(depositDate + 'T12:00:00').toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
      page.drawText(`Date : ${dateFr}`, { x: 50, y, size: 11, font, color: rgb(0.3, 0.3, 0.3) });
      ...
      for (const row of selectedRows) {
        const name = (row.memberName ?? row.userId).slice(0, 28);
      ...
      page.drawLine({ start: { x: 50, y }, end: { x: 545, y }, thickness: 0.5, color: rgb(0.7, 0.7, 0.7) });
      y -= 18;
      const countLabel = paymentMethod === 'cheque'
        ? `${selectedRows.length} chèque${selectedRows.length > 1 ? 's' : ''}`
        : `${selectedRows.length} règlement${selectedRows.length > 1 ? 's' : ''}`;
      page.drawText(countLabel, { x: 50, y, size: 11, font: fontBold });
      ...
      a.href = localUrl;
      a.download = `bordereau-${paymentMethod}-${depositDate}.pdf`;
      a.click();
      ...
      try {
        ...
        formData.append('pdf', new File([pdfBytes.buffer as ArrayBuffer], `bordereau-${paymentMethod}-${depositDate}.pdf`, { type: 'application/pdf' }));
      ...
      batch.set(doc(db, 'bankDeposits', depositId), {
        ...
        rows: selectedRows.map(r => ({
          ...
          ...(r.transferRef ? { transferRef: r.transferRef } : {}),
          ...(r.transferDate ? { transferDate: r.transferDate } : {}),
          ...(r.cashReceiptRef ? { cashReceiptRef: r.cashReceiptRef } : {}),
          ...(r.cashDate ? { cashDate: r.cashDate } : {}),
        })),
      });
      for (const row of selectedRows) {
  ...
  const emptyLabel: Record<PaymentMethod, string> = {
    ...
    <div>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit mb-6">
        {METHOD_TABS.map(tab => (
          <button key={tab.key} type="button"
            onClick={() => { setPaymentMethod(tab.key); setGenerated(false); setGenerateError(null); }}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              paymentMethod === tab.key ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'
            }`}>
            {tab.label}
          </button>
        ))}
      ...
      {loadError && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-6">
          <p className="text-sm text-red-700 font-medium">Erreur : {loadError}</p>
        </div>
      )}
      {generateError && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-6">
      ...
      {generated && (
        <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 mb-6 flex items-center justify-between">
          ...
          {blobUrl && (
            <a href={blobUrl} download={`bordereau-${paymentMethod}-${depositDate}.pdf`}
              className="text-sm text-green-700 underline font-semibold">Télécharger à nouveau →</a>
          )}
        </div>
      )}
        ...
        <div className="lg:col-span-2">
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-6 py-12 text-center">
              <p className="text-gray-400 text-sm">{emptyLabel[paymentMethod]}</p>
            </div>
          ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {paymentMethod === 'cheque' && (
                          <div className="grid grid-cols-3 gap-1.5">
                            <input type="text" placeholder="Banque" defaultValue={row.draweeBank ?? ''}
                            ...
                            <input type="text" placeholder="Ville" defaultValue={row.draweeCity ?? ''}
                              onBlur={e => updateRowMeta(row.id, 'draweeCity', e.target.value)}
                              className="border border-gray-200 rounded px-2 py-1 text-xs text-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-400" />
                            <input type="text" placeholder="N° chèque" defaultValue={row.chequeNumber ?? ''}
                        ...
                        {paymentMethod === 'transfer' && (row.transferRef || row.transferDate) && (
                          <p className="text-xs text-gray-400">
                            {[row.transferRef && `Réf : ${row.transferRef}`, row.transferDate && `Reçu le ${new Date(row.transferDate + 'T12:00:00').toLocaleDateString('fr-FR')}`].filter(Boolean).join(' · ')}
                          </p>
                        )}

                        {paymentMethod === 'cash' && row.cashReceiptRef && (
                          <p className="text-xs text-gray-400">Reçu N° {row.cashReceiptRef}</p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-3">
            ...
            <div>
              ...
              {bankAccounts.length === 0 ? (
                <p className="text-xs text-orange-600">Aucun compte configuré.{' '}
                  <Link href="/admin/settings/bank-accounts" className="underline">Ajouter →</Link></p>
              ) : (
          ...
          {selected.size > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{selected.size} règlement{selected.size > 1 ? 's' : ''}</span>
