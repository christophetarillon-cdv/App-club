...
export default function TirageAuSortPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-gray-800 mb-1">Tirage au sort</h1>
      <p className="text-sm text-gray-500 mb-6">Gagne une saison gratuite — passez cette page en plein écran pour la projeter.</p>
      ...
      {status !== 'loading' && (
        <p className="text-xs text-gray-400 mt-3">{pool.length} participant{pool.length > 1 ? 's' : ''} en lice.</p>
      )}
    </div>
  );
}
